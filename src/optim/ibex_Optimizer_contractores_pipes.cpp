...
Optimizer::Status Optimizer::optimize() {
	...
	std::ofstream info_file("/home/felipe/Desktop/bisectores_modelo_contraccion/validacion_3heuristicas_menos_parametros.txt", std::ios::app);
	...
	try {
	    ...
	    while (!buffer.empty()) {
			...
			if ((nb_cells % 500 == 0 && nb_cells >= 0) || root_node) {
				root_node = false;
