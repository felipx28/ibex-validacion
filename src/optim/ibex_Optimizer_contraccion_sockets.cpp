...
std::string build_feature_json(long call_id, const IntervalVector& box, const System& sys) {
    int variables = sys.nb_var;
    BitSet active_ctrs = sys.active_ctrs(box);
    double bigger_diam = box.max_diam();
    double lower_diam = box.min_diam();
    double box_shape = (bigger_diam > 1e-12) ? lower_diam / bigger_diam : 1.0;
    IntervalVector goal_grad = sys.goal->gradient(box);
    
    double max_diam_grad = 0.0, avg_diam_grad = 0.0, max_mag_grad = 0.0, avg_mag_grad = 0.0;
    if (!goal_grad.is_empty() && variables > 0) {
        ...
        avg_diam_grad = sum_diam / variables; avg_mag_grad = sum_mag / variables;
    }

    std::string json = "{\"id\": " + std::to_string(call_id) +
                       ", \"features\": [";
    json += std::to_string(variables) + ", ";
    json += std::to_string(active_ctrs.size()) + ", ";
    json += std::to_string(box_shape) + ", ";
    json += std::to_string(max_diam_grad) + ", ";
    json += std::to_string(avg_diam_grad) + ", ";
    json += std::to_string(max_mag_grad) + ", ";
    json += std::to_string(avg_mag_grad) + "]}";
...
Optimizer::Status Optimizer::optimize() {
	...
	try {
		 ...
		 **********************************************************/
		System sys = lfd->finder_x_taylor.sys;
        ...
        int model_decision = -1; // -1 indica que no se ha tomado ninguna decisión aún
		bool root_node = false;
	    
		while (!buffer.empty()) {
			
			//este booleano sirve para diferenciar el nodo raíz del resto
			root_node = true;
			...
			if ((nb_cells % 50 == 0 && nb_cells >= 0) || root_node) { 
				root_node = false; //aquí ya se descarta volver a procesar nuevamente el nodo raíz
				variables = n;
				active_ctrs = lfd->finder_x_taylor.sys.active_ctrs(box);
				bigger_diam = box.max_diam();
				lower_diam = box.min_diam();
				box_shape = (bigger_diam > 1e-12) ? lower_diam / bigger_diam : 1.0;
				goal_grad = lfd->finder_x_taylor.sys.goal->gradient(box);
				...
				std::string input_json = build_feature_json(call_id++, box, sys);
				...
				//std::cout << "Model decision: " << model_decision << std::endl;
			}
...
void Optimizer::report() {
    ...
    std::cout << nb_cells << " " << time << std::endl;
}
